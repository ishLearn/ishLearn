...
export const dbQueries = {
   ...
   */
  exportTableQueries: {
    media: `CREATE TABLE IF NOT EXISTS media (ID INT(255) UNSIGNED AUTO_INCREMENT PRIMARY KEY NOT NULL, filename VARCHAR(255) NOT NULL, URL VARCHAR(255) NOT NULL, filetype VARCHAR(255), uploadedDate TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP);`,
    ...
    mediaPartOfProduct: `CREATE TABLE IF NOT EXISTS mediaPartOfProduct (MID INT(255) UNSIGNED REFERENCES media(ID) ON DELETE CASCADE, PID INT(255) UNSIGNED REFERENCES products(ID) ON DELETE CASCADE);`,

    tags: `CREATE TABLE IF NOT EXISTS tags (tag VARCHAR(255) NOT NULL PRIMARY KEY);`,
    productHasTag: `CREATE TABLE IF NOT EXISTS productHasTag (tag VARCHAR(255) NOT NULL REFERENCES tags(tag) ON DELETE CASCADE, PID INT(255) UNSIGNED REFERENCES products(ID) ON DELETE CASCADE, PRIMARY KEY (tag, PID));`,
    mediaHasTag: `CREATE TABLE IF NOT EXISTS mediaHasTag (tag VARCHAR(255) NOT NULL REFERENCES tags(tag) ON DELETE CASCADE, MID INT(255) UNSIGNED REFERENCES media(ID) ON DELETE CASCADE, PRIMARY KEY (tag, MID));`,
    userHasTag: `CREATE TABLE IF NOT EXISTS userHasTag (tag VARCHAR(255) NOT NULL REFERENCES tags(tag) ON DELETE CASCADE, UID INT(255) UNSIGNED REFERENCES users(ID) ON DELETE CASCADE, PRIMARY KEY (tag, UID));`,
    ...
    supervisedBy: `CREATE TABLE IF NOT EXISTS supervisedBy (PID INT(255) UNSIGNED NOT NULL REFERENCES products(ID) ON DELETE CASCADE, TID INT(255) UNSIGNED NOT NULL REFERENCES users(ID) ON DELETE CASCADE, status ENUM(\"${getSupervisedByStatus().join(
      '", "'
    )}\") NOT NULL, grade VARCHAR(255), PRIMARY KEY (PID, TID))`,
    uploadBy: `CREATE TABLE IF NOT EXISTS uploadBy (PID INT(255) UNSIGNED NOT NULL REFERENCES products(ID) ON DELETE CASCADE, UID INT(255) UNSIGNED NOT NULL REFERENCES users(ID) ON DELETE CASCADE, PRIMARY KEY (PID, UID))`,
    rememberProject: `CREATE TABLE IF NOT EXISTS rememberProject (PID INT(255) UNSIGNED NOT NULL REFERENCES products(ID) ON DELETE CASCADE, SID INT(255) UNSIGNED NOT NULL REFERENCES users(ID) ON DELETE CASCADE, addedTime TIMESTAMP DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (PID, SID))`,
    comments: `CREATE TABLE IF NOT EXISTS comments (PID INT(255) UNSIGNED NOT NULL REFERENCES products(ID) ON DELETE CASCADE, UID INT(255) UNSIGNED NOT NULL REFERENCES users(ID), createdTime TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updatedTime TIMESTAMP DEFAULT CURRENT_TIMESTAMP, rating INT(255) NOT NULL, commentText INT(255) UNSIGNED NOT NULL REFERENCES media(ID), PRIMARY KEY(PID, UID))`,
    refreshtokens: `CREATE TABLE IF NOT EXISTS refreshtokens (token VARCHAR(255) NOT NULL PRIMARY KEY, UID INT(255) UNSIGNED NOT NULL REFERENCES users(ID) ON DELETE CASCADE, expiryDate TIMESTAMP NOT NULL)`,
    emailTokens: `CREATE TABLE IF NOT EXISTS emailTokens (token VARCHAR(255) NOT NULL PRIMARY KEY, action VARCHAR(255) NOT NULL, UID INT(255) UNSIGNED NOT NULL REFERENCES users(ID) ON DELETE CASCADE, expiryDate TIMESTAMP NOT NULL)`,
  },
}
